...
def resolve_eeg(
    ...
    this only considers streams whose hostname matches this machine
   Pass local_only=False to search the whole network instead.
    """
    this_host = socket.gethostname()
    deadline = time.monotonic() + timeout
    target = f"named {name!r}" if name else "with type 'EEG'"
    scope = f"on host {this_host!r}" if local_only else "on the network"
    print(f"Waiting for an EEG LSL stream {target} {scope} ...")
    while time.monotonic() < deadline:
        ...
        matches = [
            ...
            and (not local_only or stream.hostname() == this_host)
        ]
        if len(matches) > 1:
            print(f"Multiple matching EEG streams found {scope}:")
            for i, stream in enumerate(matches):
                print(
                    f"  [{i}] name={stream.name()!r} host={stream.hostname()!r} "
                    f"source_id={stream.source_id()!r}"
                )
            choice = input(f"Select stream index [0-{len(matches) - 1}]: ").strip()
            stream = matches[int(choice)]
        elif matches:
            stream = matches[0]
        else:
            continue
        print(
            f"Connected to {stream.name()!r} on host {stream.hostname()!r}: "
            f"{stream.channel_count()} channels at {stream.nominal_srate():g} Hz"
        )
        return StreamInlet(stream, max_buflen=30, processing_flags=0)
    raise TimeoutError(
        f"No LSL stream {target} appeared {scope} within {timeout:g} s. "
        f"If your device's stream is on another machine, pass --any-host."
    )
...
def grab(args):
    ...
    inlet = resolve_eeg(args.eeg_stream, args.timeout, local_only=not args.any_host)
    info = inlet.info()
...
def build_parser():
    ...
    common.add_argument(
        ...
        action="store_true",
        help=(
            "search the whole network for a matching stream instead of only this "
            "machine's own device (use only if your EEG device is on another PC)"
        ),
    )
